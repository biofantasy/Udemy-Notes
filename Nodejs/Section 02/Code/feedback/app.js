// app application
// 把當前模塊所有的依賴都聲明在文件模塊的最上面
// 為了讓目錄結構保持統一清晰, 所以把所有的 HTML 文件都放到 views (視圖) 目錄裡
// 為了方便統一處理靜態資源, 所以把所有靜態資源都存放在 public 目錄裡
// 資源訪問權限設定能夠通過代碼進行靈活操控
...
http
    .createServer(function (req, res) { // 简写方式，该函数会直接被注册为 server 的 request 请求事件处理函数
        ...
        if(pathname === '/'){
            ...
            // 統一處理:
            //      如果請求路徑是以 /public/ 開頭的, 則認為要獲取 public 中的某個資源 
            //      所以可以直接把請求路徑當作文件路徑來直接進行讀取
            ...
            comment.dateTime = '2019-07-15 17:11:22'
            //加到最後
